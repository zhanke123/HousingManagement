...
<div id="choose">
    ...
    <div align="center" style="margin-top: 5%">
        <%
            String[] info = {"","","","","","","",""};
            if(session.getAttribute("UnitInfo")!=null){
                Unit unit = (Unit)session.getAttribute("UnitInfo");
                info[0] = unit.getUNITACCNUM();
                info[1] = unit.getUNITACCNAME();
                info[2] = unit.getUNITADDR();
                info[3] = unit.getORGCODE();
                info[4] = unit.getUNITPHONE();
                info[5] = unit.getUNITLINKMAN();
                info[6] = unit.getUNITAGENTPAPNO();
                info[7] = unit.getREMARK();
            }
        %>
        <form action="/UnitChange" method="post">
            <div style="font-weight: bold;font-size: 20px;color: firebrick">单位信息修改</div><br>
            <div>
                <table>
                    <tr style="width: 100%">
                        <td>单位帐号：</td>
                        <td><input id="UnitNumber" name="UnitNumber" type="text" onblur="autoInfo()" placeholder="<%=info[0]%>"></td>
                        <script>
                            function autoInfo(){
                                let UnitNumber = document.getElementById("UnitNumber").value
                                window.location.href = "/autoView?UnitNumber="+UnitNumber
                            }
                        </script>
                    </tr>
                    <tr style="width: 100%">
                        <td>单位名称：</td>
                        <td>
                            <input id="UnitName" name="UnitName" type="text" placeholder="<%=info[1]%>">
                        </td>
                    </tr>
                    <tr style="width: 100%">
                        <td>单位地址：</td>
                        <td><input name="UnitAddress" type="text" placeholder="<%=info[2]%>"></td>
                    </tr>
                    <tr style="width: 100%">
                        <td>组织机构代码：</td>
                        <td><input name="Coding" type="text" placeholder="<%=info[3]%>"></td>
                    </tr>
                    <tr style="width: 100%">
                        <td>单位类别：</td>
                        <td>
                    ...
                    <tr style="width: 100%">
                        <td>联系电话：</td>
                        <td><input name="tel" type="text" placeholder="<%=info[4]%>"></td>
                    </tr>
                    <tr style="width: 100%">
                        <td>单位经办人：</td>
                        <td><input name="unitPeople" type="text" placeholder="<%=info[5]%>"></td>
                    </tr>
                    <tr style="width: 100%">
                        <td>经办人身份证号码：</td>
                        <td><input name="ownNumber" type="text" placeholder="<%=info[6]%>"></td>
                    </tr>
                    <tr>
                        <td>备注:</td>
                        <td>
                            <input name="note" type="text" placeholder="<%=info[7]%>">
                        </td>
                    </tr>
                </table>
            </div>
